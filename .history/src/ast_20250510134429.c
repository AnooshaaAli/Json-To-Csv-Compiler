...
void fillRowWithValues(Row* row, Table* table, ASTNode* objectNode) {
    ...
    printf("Filling row for table: %s\n", table->name);
    for (int i = 0; i < objectNode->childCount; i++) {
        ASTNode* pair = objectNode->children[i];
        printf("Pair type: %s, key: %s, childCount: %d\n", pair->type, pair->strVal, pair->childCount);
        if (strcmp(pair->type, "pair") == 0 && pair->strVal && pair->childCount > 0) {
            char* key = pair->strVal;
            ASTNode* val = pair->children[0];
        }
        for (int j = 0; j < table->columnCount; j++) {
            printf("Comparing with column: %s\n", table->columns[j]);
        }
    }
}
