...
void walkAST(ASTNode* node, const char* parentTable, int parentId) {
    ...
    // Handle arrays
        ...
        // Create a row for the array as a whole
        char* schemaKey = generateSchemaKey(node);
        Table* table = findOrCreateTable(schemaKey);
        ...
        // Walk through each element in the array and treat it as a new "pair"
        for (int i = 0; i < node->childCount; i++) {
            walkAST(node->children[i], table->name, row->id);  // Use array row's ID as parentId
        }
