...
void walkAST(ASTNode* node, const char* parentTable, int parentId) {
    ...
    // Handling "pair" type (key-value pairs)
        ...
        // Check value type and assign the correct value
        if (valNode->strVal) {
    ...
    // Handling "array" type (array of elements)
        ...
        if (node->childCount == 0) {
            ...
            // For each element in the array, recurse
            for (int i = 0; i < node->childCount; i++) {
                ASTNode* child = node->children[i];
                if (child == NULL) {
                ...
                walkAST(child, parentTable, parentId);  // Recurse with current array element
            }
        }
    }
}
