    ...
    for (int i = 0; i < members->childCount; i++) {
        ...
        switch (val->type[0]) {
            case 'n': { // number
                ...
                table->rows[rowIndex].columns[colIndex] = strdup(value);

                break;
            }
            case 's': // string
                table->rows[rowIndex][colIndex] = strdup(val->strVal);
                break;
            case 'b': // boolean
                table->rows[rowIndex][colIndex] = strdup(val->boolVal ? "true" : "false");
                break;
            default:
                table->rows[rowIndex][colIndex] = strdup("unsupported");
        }
    }
}
