...
char* generateSchemaKey(ASTNode* node) {
    ...
    if (strcmp(node->type, "pair") == 0) {
        ...
        if (node->strVal != NULL) {
            ...
            return strdup("defaultKey");  // Return a default key in case of NULL
        }
    }
    // If the node is not a pair, return a default value or handle as needed
...
void walkAST(ASTNode* node, const char* parentTable, int parentId) {
    ...
    // Handling "pair" type (key-value pairs)
        ...
        char* schemaKey = generateSchemaKey(node);  // Get the schema key
        Table* table = findOrCreateTable(schemaKey); // Create/find the table for the key

        // Create the row based on the schema key
    ...
    // Handling "array" type (array of elements)
        ...
        if (node->childCount == 0) {
            ...
            for (int i = 0; i < node->childCount; i++) {
                ...
                char* schemaKey = generateSchemaKey(child);  // Schema key for array element
                Table* table = findOrCreateTable(schemaKey);  // Find or create table for array element
                ...
                row->id = idCounter++;
                row->parentId = parentId;
                row->tableName = strdup(table->name);  // Set the table name
                row->keyCount = 1;
                row->keys = (char**)malloc(sizeof(char*));
                ...
                if (strcmp(child->type, "number") == 0) {
                    ...
                    row->values[0] = strdup(child->type);  // Save the type if it's complex
                }

                addRow(table, row);  // Add the row to the table
                free(schemaKey);
            }
        }
    }
