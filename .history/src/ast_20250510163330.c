...
void fillRowRecursive(Table* table, ASTNode* node, int rowIndex, const char* prefix) {
    ...
    if (strcmp(node->type, "object") == 0 && node->childCount > 0) {
        ASTNode* members = node->children[0];
        if (strcmp(members->type, "members") != 0) return;

        for (int i = 0; i < members->childCount; i++) {
            ASTNode* pair = members->children[i];
            if (!pair || strcmp(pair->type, "pair") != 0 || pair->childCount < 1) continue;

            const char* key = pair->strVal;
            ASTNode* val = pair->children[0];

            // Build full key path (e.g., outer.inner)
            char fullKey[256];
            ...
            if (strcmp(val->type, "object") == 0) {
                // Recurse into nested object
                fillRowRecursive(table, val, rowIndex, fullKey);
            } else {
                // Process primitive values
                int colIndex = getColumnIndex(table, fullKey);
                ...
                } else if (strcmp(val->type, "boolean") == 0) {
...
// Helper: Generate schema key string for grouping tables
char* generateSchemaKey(char** keys, int count) {
    int totalLength = 0;
    for (int i = 0; i < count; ++i) {
        totalLength += strlen(keys[i]) + 1; // +1 for comma or null
    }
...
// Find or create a table based on schema
Table* getOrCreateTableFromObject(ASTNode* node, const char* suggestedName) {
    char** keys = extractKeys(node);
    int keyCount = 0;
    while (keys[keyCount] != NULL) keyCount++;

    char* schemaKey = generateSchemaKey(keys, keyCount);

    // Traverse existing tables
    extern Table* allTables; // assume global linked list head
    Table* current = allTables;
    while (current) {
        char* existingKey = generateSchemaKey(current->columns, current->columnCount);
        if (strcmp(existingKey, schemaKey) == 0) {
            free(schemaKey);
            return current; // schema match found
        }
        current = current->next;
    }

    // Not found: create new table
    Table* newTable = malloc(sizeof(Table));
    newTable->name = strdup(suggestedName);
    newTable->columns = keys;
    newTable->columnCount = keyCount;
    newTable->rows = NULL;
    newTable->rowCount = 0;
    newTable->nextId = 1;
    newTable->next = allTables;
    allTables = newTable;
    return newTable;
}

// Create a new row in a table based on an object
Row* createRow(Table* table, ASTNode* node, int parentId) {
    Row* row = malloc(sizeof(Row));
    row->id = table->nextId++;
    row->parentId = parentId;
    row->columnCount = table->columnCount;
    row->values = malloc(sizeof(char*) * row->columnCount);
    row->next = table->rows;
    table->rows = row;
    table->rowCount++;

    for (int i = 0; i < row->columnCount; ++i) {
        row->values[i] = strdup(""); // default empty
    }

    if (!node || strcmp(node->type, "object") != 0 || node->childCount < 1) return row;
    ...
    for (int i = 0; i < members->childCount; ++i) {
        ...
        int colIndex = getColumnIndex(table, key);
        if (colIndex == -1) continue;

        if (strcmp(val->type, "number") == 0) {
            char buf[32];
            snprintf(buf, sizeof(buf), "%d", val->intVal);
            row->values[colIndex] = strdup(buf);
        } else if (strcmp(val->type, "string") == 0) {
            row->values[colIndex] = strdup(val->strVal);
        } else if (strcmp(val->type, "boolean") == 0) {
            row->values[colIndex] = strdup(val->boolVal ? "true" : "false");
        } else if (strcmp(val->type, "object") == 0) {
            // Recurse to extract nested table
            extractTables(val, table->name, row->id);
        }
    }
    return row;
}

// Extract tables recursively from AST
void extractTables(ASTNode* node, const char* parentTable, int parentId) {
    if (!node || strcmp(node->type, "object") != 0) return;

    Table* table = getOrCreateTableFromObject(node, parentTable ? parentTable : "root");
    Row* row = createRow(table, node, parentId);

    // Mark node with parent (optional)
    for (int i = 0; i < node->childCount; ++i) {
        ASTNode* pair = node->children[0]->children[i];
        if (pair->children[0]->type)
            pair->children[0]->parent = node; // set back-link
    }
}
...
void extractTables(ASTNode* node, const char* parentTable, int parentId) {
    ...
    if (strcmp(node->type, "object") == 0) {
        ...
        fillRowRecursive(table, node, rowIndex, "");
