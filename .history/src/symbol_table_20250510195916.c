...
// Function to expand the tables array when needed
void expandTablesArray() {
    if (tableCount >= INITIAL_TABLE_CAPACITY) {
        int newCap = INITIAL_TABLE_CAPACITY * 2;
        tables = (Table**)realloc(tables, newCap * sizeof(Table*));
        if (tables == NULL) {
            printf("Memory allocation failed while expanding tables array.\n");
...
Table* findOrCreateTable(const char* schemaKey) {
    ...
    expandTablesArray();  // Ensure we have enough space

    Table* newTable = (Table*)malloc(sizeof(Table));
    if (newTable == NULL) {
        printf("Memory allocation failed for new table.\n");
        exit(1);  // Exit if memory allocation fails
    }

    newTable->schemaKey = strdup(schemaKey);
    if (newTable->schemaKey == NULL) {
        printf("Memory allocation failed for schemaKey.\n");
        exit(1);  // Exit if memory allocation fails
    }

    newTable->name = strdup(schemaKey);  // Customize this as needed
    if (newTable->name == NULL) {
        printf("Memory allocation failed for table name.\n");
        exit(1);  // Exit if memory allocation fails
    }
